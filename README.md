...
IP(Interne Protocol)는 인터넷상에 있는 개별 네트워크끼리 패킷을 라우팅하는 프로토콜이다.

이 프로토콜을 이용하여 다른 네트워크에 있는 컴퓨터와 통신할 수 있다. L3 network 레이어의 대표 프로토콜이다.

호스트에서 호스트까지 통신을 책임진다. process data unit은 패킷이다.

### IP 주소

네트워킹이 가능한 장비(서버장비 PC, 스마트포느 테블릿PC 등등 인터넷에 연결되는 모든 장비)를 식별하는 주소

네트워크상에서 통신을 하기위해서는 몇가지 통신규약을 따라야하는데, 그러한 규약중에 "네트워킹하는 장비들에게 숫자 12개의 고유한 주소를 주어서

그 주소로 통신을 할 상대를 구분하도록 하자" 라는 규약이 존재한다. 

#### 숫자 12개의 고유한 주소가 바로 IP주소이다.

IP주소는 통신기기에 하나씩 부여되고, 그 덕에 인터넷이 가능하게 된다.

IP를 표현하는 방식은 옥텟이라는 단위로 나누게된다.
...
기술의 발전을 염두해 12자리의 10진수 숫자의 배열로 ip주소를 만듦으로서 40억개의 주소가 존재할 수 있도록 설계하였다.

하지만, 기술이 급속도로 발전하며 IP주소의 수가 부족해져서 IPv6가 등장하였다.

### IPv6

2의 128제곱개를 사용할 수 있다. 보안적인 부분도 IPv4보다 뛰어나다.

하지만 IPv6을 사용화하기 위해서는 기존의 네트워크 장비들을 모두 교체해야하는 등 너무나 많은 자금이 필요해서
...
하나의 네트워크 안에서 IP는 네트워크영역은 같아여하고, 호스트 영역은 서로 달라야 통신이 가능하다.

#### 네트워크 영역과 호스트 영역을 나누는 방법이자 약속이 클래스이다.

몇개의 호스트 IP를 가질 수 있는가에 따라서 클래스를 나눌수 있다.


IP 주소 클래스는 총 5개가 있다. A, B, C, D, E (D, E 클래스는 멀티캐스트용, 연구용으로 사용된다.)

### A 클래스

네트워크주소 호스트 주소

0xxx xxxx . xxxx xxxx . xxxx xxxx . xxxx xxxx 와 같은 형식이다.

0.0.0.0 ~ 127.255.255.255

네트워크 주소를 가질  수 있는 갯수는 2^7 개이다.

호스트 주소가 가질 수 있는 갯수는 (2^24) - 2개이다.(-2를 하는 이유는 호스트 주소 영역이 모두가 1인경우는 브로드캐스트 주소로, 모두가 0인 경우는 네트워크 주소로 사용하기 때문이다.)
...
호스트 주소 범위는 (2^8) - 2 개이다.




| 클래스| 첫 고정 비트 | 네트워크 주소영역 | 호스트 주소영역 |
| -------- | -------- | -------- | -------- |
|A| 0     | 8bit     | 24bit     |
|B| 10 | 16bit | 16bit |
|C| 110 | 24bit | 8bit |
...
* Logical Addressing : 데이터 링크 계층은 물리적 주소지정을 구현하고 네트워크 계층은 논리적 주소 지정을 구현한다.
또한 소스시스템과 대상 시스템을 구별하는데 논리 주소 지정이 사용된다. 네트워크 계층은 송신자와 수신자 모두의 논리적 주소를 포함하는 헤더를 패킷에 추가한다.
...
* ACK : 응답번호 필드가 유효한지 설정. 클라이언트가 보낸 최초의 SYN패킷 이후에 전송되는 모든 패킷은 이 플래그가 설정되어야한다.
송신측에 대하여 수식측에서 긍정 응답으로 보내지는 전송제어용 캐릭터, ACK번호를 사용하여 패킷이 도착했는지 확인한다. 송신한 패킷이 제대로 도착하지 않았으면 재송신을 요구한다.
...
| 데이터의 경계를 구분안함 | 데이터의 경계를 구분함 | Column 3 |
